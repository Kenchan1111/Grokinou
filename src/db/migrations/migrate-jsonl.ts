  ...
  for (const filePath of jsonlFiles) {
    ...
    try {
      ...
      for (let i = 0; i < sessions.length; i++) {
        ...
        const session = sessionRepo.findOrCreate(
          workdir,
          'grok', // Default provider for legacy
          'unknown', // Unknown model for legacy
        );
 ...
 */
function splitIntoSessions(lines: string[]): JSONLEntry[][] {
  const sessions: JSONLEntry[][] = [];
  let currentSession: JSONLEntry[] = [];
  let lastTimestamp = 0;

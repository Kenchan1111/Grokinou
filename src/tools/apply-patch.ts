...
interface HunkLine { type: ' ' | '+' | '-'; content: string }
interface Hunk { oldStart: number; oldCount: number; newStart: number; newCount: number; lines: HunkLine[] }
interface FilePatch { oldPath: string; newPath: string; hunks: Hunk[] }

export class ApplyPatchTool {
  private confirmationService = ConfirmationService.getInstance();

  async apply(patch: string, dryRun: boolean = false): Promise<ToolResult> {
    try {
  ...
  private resolveTargetPath(fp: FilePatch): string {
    const pick = (p: string) => p.replace(/^a\//, "").replace(/^b\//, "");
    if (fp.newPath && fp.newPath !== "/dev/null") return path.resolve(pick(fp.newPath));
    if (fp.oldPath && fp.oldPath !== "/dev/null") return path.resolve(pick(fp.oldPath));
    return path.resolve("unknown");
  }

  private parseUnifiedDiff(patch: string): FilePatch[] {
        ...
        while (i < lines.length && lines[i].startsWith("@@ ")) {
          ...
          while (i < lines.length && !lines[i].startsWith("@@ ") && !lines[i].startsWith("--- ")) {
            ...
            if (line.startsWith("+") || line.startsWith("-") || line.startsWith(" ")) {
              hunkLines.push({ type: line[0] as any, content: line.slice(1) });
            }
            i++;
          }
  ...
  private buildFromHunksCreate(fp: FilePatch): string {
    const out: string[] = [];
    for (const h of fp.hunks) {
      for (const ln of h.lines) {
        if (ln.type === '+') out.push(ln.content);
        else if (ln.type === ' ') out.push(ln.content); // tolerate spaces in create hunks
      }
    }
    return out.join("\n");
  }

  private applyHunksModify(oldText: string, fp: FilePatch): string {
